...
- 可选地同步剪贴板中的文本、RTF、HTML、图片，以及限制大小内的文件，方向跟随当前发送 / 接收 / 双向 / 自动协商模式；默认允许最多 100 MB 的剪贴板二进制内容
...
Synly 本体可以在 macOS / Linux / Windows 上编译，但当前“音频同步”依赖 Opus，并且原生音频采集 / 播放后端目前只实现了 Windows 版本。也就是说：

- 不开音频同步时，三大桌面平台都可以正常编译和使用文件 / 剪贴板同步
- 开启音频同步相关编译时，三大平台都需要准备 Opus 开发库
- 真正运行音频发送 / 接收时，目前优先按 Windows 环境准备
...
- 当前非 Windows 平台的音频后端还没有实现；如果只使用文件 / 剪贴板同步，不受影响
...
```bash
synly both . --host --max-folder-depth 2
```
