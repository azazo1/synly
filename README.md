...
- `config.toml` 保存用户设置和运行参数. 输入方向, 屏幕边缘, 热键, 启动提权, 按键映射, 滚动反向, 按住拦截开关, 游戏光标模式和自动切换开关都位于 `[input]`.
...
接收端启用游戏光标模式后, 远端 `Motion` 不再换算成绝对坐标, 而是直接注入相对增量, 使锁定光标并读取相对移动的游戏(例如 MC 的 3D 光标, FPS 视角)能够跟随控制端鼠标. 全屏本身不算捕获状态, 自动切换只在系统光标被隐藏或范围被裁剪时生效, 游戏暂停菜单或设置界面(普通鼠标状态)不会触发. 该模式为单机或无反作弊游戏设计, 带 EAC/BattlEye/Vanguard 等反作弊的联机游戏可能忽略注入输入或存在封号风险. 游戏模式下边缘返回失效, 使用紧急收回热键返回, 前台捕获状态结束时接收端会自动释放控制.
...
| 游戏光标模式和自动切换开关 | 推进 capability generation 并重建输入辅助通道 |
