...
- 自动更新由安装器整目录落地, 覆盖程序文件前会把被占用的旧映像改名让位, 服务这类无法在升级期间停掉的进程不会挡住安装. 正在运行的服务进程仍映射着更新前的映像, 应用在更新后的首次申请输入提权前会识别这种情况, 通过一次 UAC 执行 `service restart` 让服务重新加载新版本; 识别依据是交接安装器时写下的 `apply-update.pending` 落地标记, 缺少该标记时退回用让位文件 (`<name>.old`) 是否删得掉来推断. 服务重启失败或用户取消时继续沿用当前服务, 不影响本次提权.
- 安装器让位时备份名为 `<name>.old`, 该名字已被占用时改用 `<name>.old.<tick>`, 避免下一次更新因为无法让位而失败; 应用启动时统一清理已经释放的让位文件.
