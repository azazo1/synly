...
pub(super) const MAIN_CONFIG_VERSION: u32 = 1;
...
mod tests {
    ...
    fn main_v0_extracts_gui_state_and_adds_version() {
        ...
        assert_eq!(table.get("version").and_then(toml::Value::as_integer), Some(1));
    ...
    fn current_main_config_is_not_migrated() {
        let migration = migrate_main_config("version = 1\n").unwrap();
        assert!(!migration.migrated);
    ...
    fn future_versions_are_rejected() {
        ...
        assert!(migrate_main_config("version = 2\n").is_err());
    }
}
